...
class PairSynergyNet(nn.Module):
    """融合两个药物指纹与细胞上下文的 MLP 分类器。"""

    def __init__(self, drug_dim: int, context_dim: int, hidden_dims: Sequence[int], dropout: float):
        super().__init__()
    ...
    def forward(self, drug_a: torch.Tensor, drug_b: torch.Tensor, context: torch.Tensor) -> torch.Tensor:
...
def focal_loss(logits: torch.Tensor, targets: torch.Tensor, gamma: float, pos_weight: float) -> torch.Tensor:
